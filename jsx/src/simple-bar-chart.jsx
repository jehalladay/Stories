...
const SimpleBarChart = (props) => {
	...
	useEffect(() => {
		const bars = d3.select(chartArea.current)
			...
			.join(
				enter => enter.append('rect') .attr('fill', d=>`rgb(0, 0, ${Math.floor(d*3)})`)
                    .attr('height', d=>d)     .attr('width', (d, i, a) => (width/a.length)-3)
					.attr('y', d=>height-d)   .attr('x', (d, i, a)=>i*(width/a.length))
                    .attr('class', 'new'),
	...
	return (
		<Fragment>
			<svg ref = {chartArea} style={{height: height, width: width}}>

			</svg>
			<br></br>
			<button onClick={click} style={{width: `${4*width/5}px`, marginLeft: `${width/10}px`, height: '15px', fontSize:'8pt'}}>
				Get New values
			</button>
		</Fragment>
	)
